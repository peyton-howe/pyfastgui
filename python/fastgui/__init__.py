...
class DockArea:
    ...
    (dropping on the edges splits that region; dropping in the center merges the two into a new
    `Tabs` group). Drop near the *window's* own outer edge instead of on a specific panel to add
    the dragged panel as a new row/column spanning the *entire* dock area, rather than just a
    strip the size of whatever panel was under the cursor. Pass a `Tabs` instead of a `Panel` to
    `add_panel` to seed a region with several tabbed panels sharing one spot up front. To ungroup,
    grab a tab's own header segment (not just its content) and drag it out onto another region —
    down to one remaining member, the group dissolves back into a plain `Panel`.
    ...
    Known gaps (see ROADMAP.md's M6 status): dropping onto an existing `Tabs`' center (to add a
    fourth tab to an already-formed group, say) isn't supported — only forming a *new* two-tab
    group by dropping one `Panel` onto another, or growing/shrinking one via drag-out. Floating
    panels are a separate, `Window`-level feature (`Window.add_floating_panel`), not a `DockArea`
    one — a floating panel can't be dropped into or dragged out of a `DockArea` in this pass.
    ...
    def _on_rearrange(self, dragged_id: int, target_id: int, zone: str) -> None:
        """Bound to every `Panel`'s title-bar drag (see `add_panel`); fires when the user drops
        one over another region. Rebuilds `self._root` and, if this `DockArea` has already been
        given to a `Window`, asks it to re-attach the new structure — nothing else in this
        architecture gives a composite widget a route to trigger that on its own, see
        `Window.set_content`'s Rust-side doc comment."""
        if dragged_id == target_id or self._root is None:
            return
        remainder, extracted = _extract(self._root, dragged_id)
        if extracted is None:
            return  # dragged panel not found in this DockArea — nothing to do
        if remainder is None:
            # It was the only thing here; nothing to attach it near.
        ...
        if zone == "center":
            target_widget = target_leaf["widget"]
            if not isinstance(target_widget, Panel):
                return  # dropping onto an existing Tabs' center isn't supported yet
            new_group = Tabs([target_widget, extracted["widget"]])
            self._root = _replace(remainder, target_id, {"kind": "leaf", "widget": new_group})
        else:
            direction = "row" if zone in ("left", "right") else "column"
            if zone in ("right", "bottom"):
            ...
            self._root = _replace(remainder, target_id, new_split)
    ...
    def _fastgui_widget(self):
        ...
        if self._root is None:
            raise RuntimeError("DockArea has no panels — call add_panel(...) before set_content(...)")
