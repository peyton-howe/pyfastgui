...
/// Hit-test margin (logical points) for floating-window edge/corner resize.
const FLOAT_RESIZE_MARGIN: f32 = 6.0;
/// Minimum floating-window inner size in logical points.
const MIN_FLOAT_WIDTH: f32 = 160.0;
const MIN_FLOAT_HEIGHT: f32 = 100.0;

/// How far (logical points) a docked-panel drag must move before the tear-off ghost appears.
...
impl App {
    ...
    fn add_floating_panel(
        ...
        let renderer = MetalRenderer::new(&window, physical.width, physical.height)?;
    ...
    fn upload_floating_content(floater: &mut FloatingWindow) -> Result<(), MtlRendererError> {
    ...
    fn spawn_tear_ghost(&mut self, event_loop: &ActiveEventLoop) -> Result<(), RunError> {
        ...
        let physical = window.inner_size();
        let logical = physical.to_logical::<f64>(scale_factor.max(0.01));
        let mut renderer = MetalRenderer::new(&window, physical.width, physical.height)?;
